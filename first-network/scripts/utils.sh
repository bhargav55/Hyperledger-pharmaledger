...
installChaincode() {
  ...
  VERSION=${3:-1.0}
  set -x
  peer chaincode install -n medicinecontract -v ${VERSION} -l ${LANGUAGE} -p ${CC_SRC_PATH} >&log.txt
  res=$?
  set +x
  cat log.txt
...
instantiateChaincode() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer chaincode instantiate -o orderer.pharma-network.com:7050 -C $CHANNEL_NAME -n medicinecontract -l ${LANGUAGE} -v ${VERSION} -c '{"Args":["init"]}'  >&log.txt
    res=$?
    set +x
  else
    set -x
    peer chaincode instantiate -o orderer.pharma-network.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n medicinecontract -l ${LANGUAGE} -v 1.0 -c '{"Args":["init"]}'  >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
...
upgradeChaincode() {
  ...
  set -x
  peer chaincode upgrade -o orderer.pharma-network.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n medicinecontract -v 2.0 -c '{"Args":["init"]}' 
  res=$?
  set +x
  cat log.txt
